...
This is an extension of the below project where I have made modifications to allow for serial control of the radio device (via COM port). At the moment it is still a work in progress that is almost done. Current caveat is the radio needs to be on FM band before trying to do any FM freq requests. Although I'm sure it won't be too much trouble to add functionality for changing bands, it is not implemented right now. Especially since my underlying purpose is to only control a FM radio via UART/COM (which hopefully will come from an ESP32 that is controlled by home assistant - I will upload an ESPHome example that is functioning when done). Other functionality currently working is Seek Up, Seek Down, Volume and Mute.

To control via the COM port you need to have CR-LF line endings active and baud set to 57600 (although you can change this in the defs.h file). The first character is a 'control command' which must match the eSerialCtlCmds in the defs.h file (0 thru 6 are the current command numbers). What follows depends on the control command. So for example the FM frequency tuning...the remaining characters are the frequency. So 102.10 FM would be 010210[CR-LF] since the control command for 0 is FM Freq.

Yes...to get serial control you HAVE to give up something and that something is RDS. As long as you don't care about RDS information (which in my case since the radio will be out of sight it has no use), there is enough space to fit full serial control into the program code. You cannot have both though so don't ask me to try...
