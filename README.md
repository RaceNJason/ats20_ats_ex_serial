...
This is an extension of the below project where I have made modifications to allow for serial control of the radio device (via COM port). Although now considered complete, since the AM and SBW bands are not implemented it could still be considered a work in progress that is almost done. Current caveat is the radio needs to be on FM band before trying to do any FM freq requests (haven't even tested what happens if it isn't). Although I'm sure it won't be too much trouble to add functionality for changing bands, it is not implemented right now and for the moment I don't plan to. Especially since my underlying purpose is to only control a FM radio via UART/COM which is now complete and functioning (I've included the yaml code as well). Other functionality currently working is Seek Up, Seek Down, Volume and Mute along with feedback for local changes and/or scanning (so the change gets reflected in the home assistant ui).

To control via the COM port you need to have CR-LF line endings active and baud set to 57600 (although you can change this in the defs.h file). The first character is a 'control command' which must match the eSerialCtlCmds in the defs.h file (0 thru 6 are the current command numbers). What follows depends on the control command. So for example the FM frequency tuning...the remaining characters are the frequency. So 102.10 FM would be 010210[CR-LF] since the control command for 0 is FM Freq. You can look at the yaml code for example (or just use it directly). Note that this requires a direct wiring from UART pins to UART pins. You cannot use the USB port/cable since the UART to USB chips are slave devices only (two slaves cannot communicate with each other).

Yes...to get serial control you HAVE to give up something and that something is RDS and battery monitoring. As long as you don't care about RDS information (which in my case since the radio will be out of sight it has no use and the battery monitoring also shouldn't be an issue since it should be continually plugged into a power source in this use case). Anyway, without those features there just enough space to fit full serial control into the program code. You cannot have both though so don't ask me to try...
